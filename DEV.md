...
api/
  ...
  historical-price.js  Vercel Serverless Function：取得歷史股價（用於 ROI 趨勢圖）
functions/
  api/                 Cloudflare Pages Functions 版本：與 api/*.js 邏輯相同，僅 handler 簽章改用 Cloudflare 的
                        onRequestGet({ request, env })／Response 寫法（Vercel 用的是 (req, res) callback）
db/
...
### 方式三：全部部署在 Cloudflare Pages（免費，含 API）

跟方式一類似，是最簡單的單一服務方案，差別是改用 Cloudflare 的免費方案（Pages 靜態託管 + Pages Functions 無伺服器函式），兩者都在同一個網域下，不需要處理 CORS。`functions/api/*.js` 是 `api/*.js` 的 Cloudflare Pages Functions 版本，邏輯完全相同。

1. **註冊 Cloudflare 帳號**（免費）：<https://dash.cloudflare.com/sign-up>。
2. **建立 Pages 專案並連結本 repo**：Cloudflare Dashboard → Workers & Pages → Create → Pages → Connect to Git，選擇這個 GitHub repo。
3. **建置設定**：本專案沒有建置步驟，設定如下即可：
   - Framework preset：`None`
   - Build command：留空
   - Build output directory：`/`（repo 根目錄）

   Cloudflare 會自動偵測到 `functions/` 目錄並把裡面的檔案部署成 Pages Functions，不需要額外設定；`functions/api/stock-price.js` 會對應到 `/api/stock-price` 路徑，`functions/api/historical-price.js` 對應到 `/api/historical-price`，與 `src/stockPrice.js`、`src/historicalPrice.js` 呼叫的路徑一致。

4. **部署**：儲存後 Cloudflare 會立即開始第一次部署，完成後會取得一個網域，例如 `https://portfolio-dashboard.pages.dev`，之後每次 push 到指定分支都會自動重新部署。
5. **（選用）CORS 白名單**：因為前端與 API 同網域，瀏覽器不會帶 `Origin` 標頭，`functions/api/*.js` 內建的白名單檢查不會擋到這種同網域請求，不需要額外設定。若之後想讓其他網域（例如自訂網域以外的來源）呼叫這個 API，到 Pages 專案的 Settings → Environment variables 設定 `ALLOWED_ORIGINS`（逗號分隔多個來源）即可，不需要改程式碼。
6. **（選用）自訂網域**：Pages 專案的 Settings → Custom domains 可以掛上自己的網域，一樣完全免費。

**本機測試**：安裝 [Wrangler CLI](https://developers.cloudflare.com/workers/wrangler/)（`npm install -g wrangler` 或用 `npx`），在 repo 根目錄執行 `npx wrangler pages dev .`，即可在本機同時測試靜態頁面與 `functions/api/*.js`。

**免費額度**（寫作當下的 Cloudflare Pages Free plan，實際請以官方公告為準）：每月 500 次建置、無限頻寬與請求數的靜態託管、Pages Functions 每日 100,000 次請求；對個人使用的投資儀表板來說相當充裕。

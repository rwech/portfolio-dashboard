...
const { parseCsv, stringifyCsv, fetchInitialCsv, fetchExampleCsv, downloadCsv, fileNameFor } = window.PFD.csv;
...
describe('csv.parseCsv', () => {
  ...
  it('rejects a negative fee but allows a blank fee to default to 0', () => {
    const negativeFee = parseCsv('date,symbol,name,action,quantity,price,fee\n2024-01-10,2330,,buy,10,560,-5\n', 'TW');
    expect(negativeFee.errors[0].reason).toMatch(/fee/);

    const blankFee = parseCsv('date,symbol,name,action,quantity,price,fee\n2024-01-10,2330,,buy,10,560,\n', 'TW');
...
describe('csv.stringifyCsv', () => {
  ...
  it('produces just the header row when given no transactions', () => {
    expect(stringifyCsv([])).toBe('date,symbol,name,action,quantity,price,fee\n');
  });

  it('treats a missing field as an empty string instead of the literal "undefined"', () => {
    const tx = { date: '2024-01-10', symbol: '2330', action: 'buy', quantity: 1, price: 1, fee: 0 };
    expect(stringifyCsv([tx])).toBe('date,symbol,name,action,quantity,price,fee\n2024-01-10,2330,,buy,1,1,0\n');
  });
});
...
describe('csv.fetchInitialCsv / fetchExampleCsv', () => {
  ...
  it('parses the response body when the fetch succeeds', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      text: async () => 'date,symbol,name,action,quantity,price,fee\n2024-01-10,2330,,buy,1,1,0\n',
    }));
